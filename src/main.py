...
def init_resources(args):
    ...
    """
    WordEmbeddings.from_file(args.wvecs, mmap_fname=args.wvecs.name + ".mmap")
...
def do_train(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        logging.info("train error: %.3f", scorer)
...
#    command_parser.add_argument('--output', type=argparse.FileType('w'), default="{rundir}/output", help="Type of model to use.")
    command_parser.add_argument('--output', type=argparse.FileType('w'), default="output", help="Type of model to use.")
